...
CONTRACT_DATA_PATTERN = "SMC-合同签约数据-媒介*.xlsx"
...
def fmt_count(value) -> str:
    return f"{int(round(to_float(value))):,}份"
...
def resolve_contract_data_path(explicit_path: str | None = None) -> Path | None:
    candidates = []
    if explicit_path:
        candidates.append(Path(explicit_path).expanduser().resolve())

    base_dir = Path(__file__).resolve().parent
    for folder in [base_dir.parent, base_dir, Path.cwd()]:
        candidates.extend(sorted(folder.glob(CONTRACT_DATA_PATTERN), reverse=True))
...
def infer_contract_cutoff(path: Path) -> str:
...
def load_contract_summary(path: Path) -> dict:
    ...
    total_row = next(
        (row for row in rows if str(row.get("供应商媒介") or "").strip().lower() == "total"),
        rows[0] if rows else {},
    )
    completion_value = to_percent_number(total_row.get("总完成率"))
    pending_value = to_float(total_row.get("签署中合同数")) + to_float(total_row.get("填写中合同数"))
    rejected_value = to_float(total_row.get("博主拒签合同数")) + to_float(total_row.get("已作废合同数"))
    return {
        "id": "electronic-contract",
        "title": "电子合同",
        "summaryTitle": f"行业二组电子合同签署情况汇总（截止{infer_contract_cutoff(path)}）",
        "summaryMetrics": [
            {"label": "总完成率", "value": f"{completion_value:.2f}%"},
            {"label": "总共触发合同", "value": fmt_count(total_row.get("触发合同数"))},
            {"label": "待签署", "value": fmt_count(pending_value)},
            {"label": "已完成签署", "value": fmt_count(total_row.get("签约完成合同数"))},
            {"label": "拒签/作废", "value": fmt_count(rejected_value)},
        ],
        "noteLabel": "原因/跟进说明",
        "note": "",
        "removable": False,
...
def calc_non_sza_customers(rows):
    scope = [row for row in rows if not row["is_sza"]]
    grouped = {}
    for row in scope:
        advertiser = row["advertiser"] or "未填写客户"
    ...
    customers = sorted(
        grouped.values(),
        key=lambda item: (
            (item["gross_profit"] / item["actual_income_no_tax"]) if item["actual_income_no_tax"] else 0,
            -item["exec_price_tax"],
        ),
    )
    table = []
...
def build_special_items(contract_path: Path | None) -> list[dict]:
    items = []
    if contract_path and contract_path.exists():
        items.append(load_contract_summary(contract_path))
    else:
        items.append(
            {
                "id": "electronic-contract",
                "title": "电子合同",
                "summaryTitle": "行业二组电子合同签署情况汇总",
                "summaryMetrics": [],
                "noteLabel": "原因/跟进说明",
                "note": "",
                "removable": False,
...
def build_json(confirm_rows, execute_rows, confirm_path: Path, execute_path: Path, period: str, title: str, feedback_path: Path, yellow_headers: list[str], contract_path: Path | None = None):
    ...
    non_sza_customers = calc_non_sza_customers(execute_rows)
    low_margin_orders = build_low_margin_orders(execute_rows)
    low_margin_summary = summarize_low_margin_orders(low_margin_orders)
    special_items = build_special_items(contract_path)
    risk_item_count = 1 + len(special_items)
    ...
    data = {
        ...
        "metrics": [
            ...
            {"section": "risk", "label": "待协调事项", "value": f"{risk_item_count}项", "note": "支持周会记录整理与专项事项补充。"},
        ],
        "summaries": [
            {"title": "经营表现", "copy": "经营模块已经按 SZA 与非SZA 自动拆分，成交金额已切到执行订单口径。"},
            {"title": "专项推进", "copy": "第二大项已改成低毛利专项，低毛利订单来自执行订单中预估订单毛利率低于14%的数据。"},
            {"title": "事项管理", "copy": "风险事项可直接粘贴周会记录自动整理，电子合同、自动计提和返点进度可在同页持续补充。"},
        ],
        "highlights": [
        ...
        "sections": {
            "core": {
                ...
                "nonSza": {
                    ...
                    "customers": non_sza_customers,
                    "narratives": [{"title": "非SZA经营结论", "copy": "非SZA 上方核心指标汇总全部非华为订单，客户清单按预估毛利率从低到高排序，方便优先关注低毛利客户。"}],
                },
...
def main():
    ...
    parser.add_argument("--contract-xlsx", default="", help="电子合同汇总 Excel 路径，可选")
    ...
    contract_path = resolve_contract_data_path(args.contract_xlsx or None)
    ...
    data = build_json(confirm_rows, execute_rows, confirm_path, execute_path, args.period, args.title, feedback_path, yellow_headers, contract_path=contract_path)
    ...
    if contract_path:
        print(f"Contract data: {contract_path}")

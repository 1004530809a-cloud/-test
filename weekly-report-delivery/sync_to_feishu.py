...
BITABLE_ATTACHMENT_FIELD_TYPE = 17
...
def list_fields(app_token: str, table_id: str, access_token: str, view_id: str = "", page_token: str = ""):
    query = {"page_size": 100}
    if view_id:
        query["view_id"] = view_id
    if page_token:
        query["page_token"] = page_token
    url = f"{API_ROOT}/bitable/v1/apps/{app_token}/tables/{table_id}/fields"
    if query:
        url = f"{url}?{parse.urlencode(query)}"
    return http_json("GET", url, headers={"Authorization": f"Bearer {access_token}"})
...
def get_field_type_label(field_type: int) -> str:
    ...
    return labels.get(field_type, f"未知类型({field_type})")


def find_field_meta(app_token: str, table_id: str, access_token: str, field_name_or_id: str, view_id: str = "") -> dict | None:
    ...
    page_token = ""
    while True:
        data = list_fields(app_token, table_id, access_token, view_id=view_id, page_token=page_token)
        if data.get("code") != 0:
            raise RuntimeError(f"读取字段失败: {data}")
        for item in data.get("data", {}).get("items", []) or []:
            field_name = str(item.get("field_name") or "").strip()
            field_id = str(item.get("field_id") or "").strip()
            if target not in {field_name, field_id}:
                continue
...
def validate_snapshot_attachment_field(config: dict, access_token: str) -> dict | None:
    ...
    meta = find_field_meta(config["app_token"], table_id, access_token, attachment_field)
    if not meta:
        raise RuntimeError(f'周报快照表未找到字段“{attachment_field}”。请确认飞书字段名与 FEISHU_SYNC_CONFIG_JSON 完全一致。')
    if meta["type"] != BITABLE_ATTACHMENT_FIELD_TYPE:
        current_type = get_field_type_label(meta["type"])
        raise RuntimeError(
            f'周报快照表字段“{attachment_field}”当前类型为 {current_type}（field_type={meta["type"]}），不是附件字段。'
            "请在飞书多维表格里把它改成“附件”后重试。"
        )
    return meta
